...
#[derive(Debug, Error)]
pub enum PngError {
    #[error("Failure generating Png data")]
    PngError,
    #[error("Unable to remove non-existent ChunkType")]
    ChunkTypeNotFound,
}
pub struct Png {
    pub header: [u8; 8],
...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, PngError> {
        for (index, c) in self.chunks.iter().enumerate() {
            let str = String::from_utf8(c.chunk_type.bytes.to_vec()).unwrap();
            if str == chunk_type {
                return Ok(self.chunks.remove(index));
            }
        }
        Err(PngError::ChunkTypeNotFound)
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        u8_vec.extend_from_slice(&self.header);
        for chunks in &self.chunks {
            u8_vec.extend(chunks.as_bytes())
        }
        u8_vec
    }

    pub fn split_raw_data_into_chunks(data: &[u8]) -> Vec<&[u8]> {
        let mut subslices = Vec::new();
        let mut index = 0;

        while index < data.len() {
            // Take first 4 bytes and convert into u32
            let slice_length_marker = u32::from_be_bytes([
                data[index],
                data[index + 1],
                data[index + 2],
                data[index + 3],
            ...
            // Calculate the total number of bytes for current subslice
            let total_bytes = (slice_length_marker + 12) as usize;

            // Take the subslice of the calculated size
            let subslice = &data[index..index + total_bytes];

            // Add the subslice to the vector
            subslices.push(subslice);

            // Move index to next subslice
            index += total_bytes
        }
        subslices
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        // Make sure starts with PNG_HEADER
        let header: &[u8; 8] = &[
            value[0], value[1], value[2], value[3], value[4], value[5], value[6], value[7],
        ];
        if header != &Self::PNG_HEADER {
            return Err(Self::Error::PngError);
        };

        let non_header_data = &value[8..];
        let mut validated_chunks: Vec<Chunk> = Vec::new();

        let raw_chunks = Self::split_raw_data_into_chunks(non_header_data);
        for chunks in raw_chunks {
            match Chunk::try_from(chunks) {
                Ok(val) => validated_chunks.push(val),
                Err(_) => return Err(Self::Error::PngError),
            }
        }
...
impl Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "Png-header: {:?}", self.header).unwrap();
        write!(f, "Png-data: {:?}", self.chunks).unwrap();
        Ok(())
    }
}
...
mod test {
    ...
    fn chunk_from_strings(chunk_type: &str, data: &str) -> Result<Chunk, PngError> {
        ...
        let chunk_type = match ChunkType::from_str(chunk_type) {
            Ok(e) => e,
            Err(e) => return Err(PngError::PngError),
        };
    ...
    fn test_valid_from_bytes() {
        ...
        let bytes: Vec<u8> = Png::PNG_HEADER
            .iter()
